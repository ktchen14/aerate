...
@adjuster.rule("ref", within="highlight")
...
@adjuster.rule("sp", within="highlight")
...
@adjuster.rule("htmlonly", "manonly", "rtfonly", "latexonly", "docbookonly")
def remove_unused_inline(self, cursor):
    """Remove a ``...only`` node unless it's ``xmlonly``."""
    return cursor.remove()


@adjuster.rule(*INLINE_TAGS, when="./*")
...
@adjuster.rule(*INLINE_TAGS, when=lambda node: node.text)
...
@adjuster.rule(*INLINE_TAGS, unless=lambda node: node.text or len(node))
def remove_null_inline(self, cursor):
    """Remove an inline markup node with no text or children."""
    return cursor.remove()


@adjuster.rule("simplesect", unless=lambda node: node.text or len(node))
def remove_null_simplesect(self, cursor):
    """Remove a ``simplesect`` node with no text or children."""
    return cursor.remove()


@adjuster.rule("simplesect", when="preceding-sibling::*[1][self::simplesect]")
...
@adjuster.rule("para", unless=lambda node: node.text or len(node))
